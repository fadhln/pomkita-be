...
Status: draft v12 (BUILD TARGET — resolusi review putaran 10; loop maks 25 putaran)
...
- policy_snapshots: snapshot_set model — policy_snapshot_sets (org_id, station_id, shift_id, set_id PK, created_at) + policy_snapshot_items (item_id PK, set_id, policy_kind: threshold|evidence, policy_id, rev_id, scope, canonical payload jsonb (payload utuh bundle per kind — satu item per kind), hash sha256; UNIQUE(set_id, policy_kind); UNIQUE(rev_id) dalam set). shift_reports.policy_snapshot_set_id → FK komposit (org_id, station_id, shift_id, set_id). Tombstone: resolusi henti pada revisi disabled → aturan tidak berlaku.
...
- loss_exception: (org_id, station_id, shift_report_id, loss_logical_id; FK komposit ke loss_entries versi) — scoped PER REPORT VERSION (DEFINISI RESMI; tidak boleh hanya loss_id), INSERT-only.
...
- ack_decisions: APPEND-ONLY penuh. ack_decisions (id PK, org_id, station_id, shift_id, report_id, version_no; FK komposit (org_id, station_id, shift_id, report_id) → shift_reports + prosedur cek version match; ack_seq integer, decision: acked|rejected, actor, decided_at, rejection_reason, is_superadmin, is_break_glass, break_glass_reason). ack_supersessions (id PK, org_id, station_id, shift_id, superseded_ack_id FK komposit, replacement_report_id FK (wajib = report current BERIKUTNYA), reason, created_at; UNIQUE(superseded_ack_id) — satu supersession per ack; INSERT-only). aktif = ack tanpa supersession row. ack_seq dialokasikan di prosedur dgn shift-row lock; tepat SATU aktif ditegakkan prosedur + deferred trigger; supersession + pembuatan report baru = SATU transaksi atomik. JWT validation lengkap: signature, exp (skew 60s), iat (tidak di masa depan, skew 60s), iss wajib sama, aud wajib sama, jti belum di-blacklist, alg == HS256 (kid di-resolve ke key registry: key aktif + prev; prev retired setelah masa grace = max exp token lama). Session issuance: satu jti per login; logout = set revoked_at.
...
- numeric types: meter numeric(10,1) dengan CHECK 0≤nilai≤meter_max; volume numeric(8,2) CHECK ≥0; uang numeric(14,0) — nilai maksimum 99.999.999.999.999 (≈ 99,9 triliun rupiah, cukup untuk MVP); harga master numeric(14,0) (bukan integer — konsisten dgn tipe uang); semua perhitungan numeric (bukan float); boundary tests: half-up .5, meter_max, overflow pengali, JSON/TS konversi (string di JSON, hindari float).
...
- baseline awal: nozzle_baseline (org_id, station_id, nozzle_id, initial_meter_value, created_by, approved_by Owner, approved_at, status pending|approved; CHECK: initial_meter_value dalam range [0, meter_max]; UNIQUE(org_id, station_id, nozzle_id) — konvensi tenant; INSERT-only: revisi baseline = baris baru; reset without prior locked report BOLEH hanya jika baseline approved ada). Backfill invariants: backfill_status='approved' ⇔ backfill_approver & backfill_approved_at & backfill_reason NOT NULL; carried-forward: is_carried_forward=true ⇔ source kolom NOT NULL (CHECK).
...
- gate matrix format (tiap baris: workflow, uji wajib, pass threshold, bukti test, rollback action):
...
| Isolasi | negative-test matrix hijau di CI | 0 pelanggaran | block deploy |
| Backfill | 1 backfill idempoten | retry = 1 report | - |
| Backup/restore | 1 restore sukses | data identik (checksum) | - |
| Konkurensi (baru) | takeover race, ack supersession, pointer isolation, policy tombstone, audit outage, reset baseline, alert clear | semua sesuai §4.2 gate | block deploy |
| JWT rotasi (baru) | token lama valid selama grace, lalu ditolak | 0 diterima setelah retire | - |
