...
Status: draft v14 (BUILD TARGET — resolusi review putaran 12; loop maks 25 putaran)
...
- alert_events (append-only penuh): event_id UUID PK, org_id, station_id, alert_rule_id FK komposit (org_id, station_id, alert_rule_id) → parent UNIQUE(alert_rules), alert_key, shift_id nullable (subject), period_start timestamptz, period_bucket date (GENERATED kolom: date_trunc('hour', period_start)::date — kolom fisik, deterministik UTC), occurrence_number, event_type: fired|cleared, source_event_id nullable. RULES: partial UNIQUE(org_id, station_id, alert_key, shift_id, period_bucket) WHERE event_type='fired' (null shift dianggap nilai khusus via coalesce dalam generated subject kolom subject_shift_id coalesce(shift_id, '00000000-0000-0000-0000-000000000000')); partial UNIQUE(source_event_id) WHERE event_type='cleared'; prosedur validasi: cleared → source wajib fired, tenant sama, alert_key sama (src_event_id NOT NULL ketika cleared — CHECK). DEDUP vs RE-ALERT TERPISAH: dedup = dalam period_bucket (scheduler retry ≤ 1 jam sama); re-alert = 24 jam → period_start baru → bucket baru → occurrence baru.
