...
Status: draft v16 (BUILD TARGET — resolusi review putaran 14; loop maks 25 putaran)
...
- RLS + authorization (MODEL JELAS): role matrix — app_readonly (EXECUTE view ber-RLS; view security_invoker + owner view punya GRANT SELECT base via view-owner, bukan ke app role), app_writer (EXECUTE SECURITY DEFINER prosedur SAJA, tanpa SELECT/INSERT base). SECURITY DEFINER berjalan sebagai owner table (BUKAN BYPASSRLS superuser); prosedur men-cek tenant secara eksplisit di dalam (row_security=off hanya agar query internal tidak dobel-filter; tenant cek tetap dilakukan manual). Tabel langsung tidak dapat diakses role app dalam bentuk apa pun.
...
- LOCK ORDER GLOBAL (total order, SEMUA resource diranking): 1) stations row → 2) shifts row → 3) shift_drafts → 4) submit_idempotency → 5) shift_reports current → 6) policy_snapshot sets → items → 7) alert_rules row → 8) audit chain org lock → 9) resource lain (tabel target: reset, baseline, evidence, dsb. — ranked leksikografis per tabel). Multi-row: selalu lock dalam urutan ascending dari ranking ini. Deadlock tests di CI.
...
- audit_log hash chain: kolom prev_hash, org_sequence bigint (per-org; di-alokasikan DI DALAM append lock per-org — bukan dari created_at), row_hash = sha256(RFC8785 canonical bytes dari kolom event + prev_hash); genesis: org_sequence=1 dgn prev_hash = 32 byte nol; UNIQUE(org_id, org_sequence); verifier: baca berurutan by org_sequence → recompute chain → 0 gap; tamper test. audit_denied TIDAK bagian chain (tersendiri).
...
- Lifecycle lengkap (SATU definisi otoritatif): shift status enum = open|submitting|failed|abandoned|awaiting_confirmation|needs_correction|locked. TRANSISI TUNGGAL: open → submitting; submitting → awaiting_confirmation | failed; failed → open (retry/REOPEN) | abandoned (auto >24 jam scheduler); awaiting_confirmation → locked (acked) | needs_correction (rejected); needs_correction → awaiting_confirmation (amendment approved; hanya jika ada report). Active-shift unique: open|submitting|failed|awaiting_confirmation. AMENDMENT ELIGIBILITY (satu aturan): amendment BISA diajukan & di-approve terhadap report status SUBMITTED atau LOCKED — approval pada locked base report menghasilkan versi baru + supersede ack lama; segment "hanya SUBMITTED" dihapus (perbaikan aturan).
