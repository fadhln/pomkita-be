...
Status: draft v15 (BUILD TARGET — resolusi review putaran 13; loop maks 25 putaran)
...
- source-reading FK: parent UNIQUE(org_id, station_id, shift_id, report_id, reading_id) pada dispenser_readings; FK (org_id, station_id, shift_id, source_shift_report_id, source_reading_row_id) komposit exact.
...
- AMENDMENT ELIGIBILITY (SATU aturan): amendment approval hanya pada report status SUBMITTED (sebelum ack). Report LOCKED hanya bisa dikoreksi via amendment → versi baru; ack lama di-supersede. ( Ini aturan tunggal — definisi lain dihapus.)
...
- alert_events (append-only penuh): event_id UUID PK, org_id, station_id, alert_rule_id FK komposit (org_id, station_id, alert_rule_id) → parent UNIQUE(alert_rules), alert_key, shift_id nullable (subject), subject_shift_id GENERATED kolom: coalesce(shift_id, '00000000-0000-0000-0000-000000000000')::uuid (NOT NULL), period_start timestamptz, period_bucket timestamptz GENERATED (kolom fisik, IMMUTABLE dgn timezone eksplisit): date_bin('1 hour', period_start AT TIME ZONE 'UTC', timestamp '1970-01-01 00:00:00+00' AT TIME ZONE 'UTC') — bucket per jam UTC; occurrence_number, event_type: fired|cleared, source_event_id nullable. RULES (SATU constraint canonical utk fired): partial UNIQUE(org_id, station_id, alert_key, subject_shift_id, period_bucket) WHERE event_type='fired'; partial UNIQUE(source_event_id) WHERE event_type='cleared'; CHECK: cleared ⇔ source_event_id NOT NULL (dengan skew: cleared boleh bertipe src fired saja — prosedur). DEDUP vs RE-ALERT TERPISAH: dedup = dalam bucket (retry scheduler ≤ 1 jam sama); re-alert = 24 jam → period_start baru → bucket jam baru.
...
- LOCK ORDER GLOBAL (total order, satu urutan utk SEMUA transaksi: 1) stations row (harga/mapping/open) → 2) shifts row → 3) shift_drafts → 4) submit_idempotency → 5) shift_reports current → 6) policy_snapshot sets/items → 7) alert_rules row → 8) tabel target lain. Semua prosedur memperoleh lock dalam urutan ini; deadlock tests di CI.
...
- Canonical JSON hash: RFC 8785 (JCS) — key terurut leksikografis, number I-JSON, no duplicates, UTF-8; hash atas payload TERNORMALISASI server (bukan raw klien); uji: same-key/same-payload → same hash, changed-payload → 409, heartbeat fencing, stale takeover, failed-retry/new-key.
- dispenser_nozzle_map exclusion: EXCLUDE per nozzle (satu nozzle hanya satu dispenser aktif) DAN per pasangan (dispenser_id, nozzle_id) — BUKAN per dispenser tunggal (1 dispenser boleh >1 nozzle).
- audit_log hash chain: kolom prev_hash (hash event sebelumnya), row_hash = sha256(canonical event bytes + prev_hash); chain scope = per org (org_id); ordering: created_at + sequence per org (dalam prosedur append, lock per-org); verifier: export → recompute chain → 0 gap; tamper test: ubah satu event → chain break terdeteksi.
...
- Lifecycle lengkap (SATU definisi): shift status enum = open|submitting|failed|abandoned|awaiting_confirmation|needs_correction|locked. Transisi: open → submitting; submitting → awaiting_confirmation | failed; failed → open (retry/REOPEN) | abandoned (auto >24 jam scheduler); awaiting_confirmation → locked (acked) | needs_correction (rejected); needs_correction → awaiting_confirmation (amendment approved; hanya jika ada report). Active-shift unique: open|submitting|failed|awaiting_confirmation. AMENDMENT ELIGIBILITY (satu aturan): approval hanya pada report SUBMITTED; report LOCKED dikoreksi via amendment → versi baru (ack lama di-supersede).
