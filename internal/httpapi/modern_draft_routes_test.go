...
func TestModernClaimDraftRoute_UsesVerifiedActorScope(t *testing.T) {
	orgID, stationID, actorID, shiftID, draftID := uuid.New(), uuid.New(), uuid.New(), uuid.New(), uuid.New()
	service := &modernDraftStub{result: appdraft.ClaimResult{DraftID: draftID, ClaimToken: uuid.New(), ClaimExpiresAt: time.Date(2026, 9, 14, 4, 0, 0, 0, time.UTC), Revision: 2}}
	sessions := &modernSessionStub{view: SessionView{OrgID: orgID, UserID: actorID, StationIDs: []uuid.UUID{stationID}}}
	router := NewRouterWithDependencySet("test", nil, RouterDependencies{Readiness: readyStub{current: true}, Verifier: verifierStub{}, Sessions: sessions, ModernDraft: service, LatestMigration: 11})
...
type modernDraftStub struct {
	request appdraft.ClaimRequest
	result  appdraft.ClaimResult
}

func (s *modernDraftStub) Claim(_ context.Context, request appdraft.ClaimRequest) (appdraft.ClaimResult, error) {
	s.request = request
	return s.result, nil
}

var _ ModernDraftService = (*modernDraftStub)(nil)
