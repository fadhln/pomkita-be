...
func TestModernPolicyRevisionRoute_UsesVerifiedOwnerScope(t *testing.T) {
	orgID, stationID, actorID := uuid.New(), uuid.New(), uuid.New()
	service := &modernPolicyStub{result: apppolicy.PolicyRevision{RevisionID: uuid.New(), PolicyID: uuid.New(), PolicyKind: "threshold", ValidFrom: time.Date(2026, 9, 14, 0, 0, 0, 0, time.UTC)}}
	sessions := &modernSessionStub{view: SessionView{OrgID: orgID, UserID: actorID, Roles: []string{"Owner"}, StationIDs: []uuid.UUID{stationID}}}
	router := NewRouterWithDependencySet("test", nil, RouterDependencies{Readiness: readyStub{current: true}, Verifier: verifierStub{}, Sessions: sessions, ModernPolicy: service, LatestMigration: 11})
...
type modernPolicyStub struct {
	request apppolicy.PolicyRevisionRequest
	result  apppolicy.PolicyRevision
}

func (s *modernPolicyStub) CreateRevision(_ context.Context, request apppolicy.PolicyRevisionRequest) (apppolicy.PolicyRevision, error) {
	s.request = request
	return s.result, nil
}

var _ ModernPolicyService = (*modernPolicyStub)(nil)
