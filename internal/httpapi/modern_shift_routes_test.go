...
func TestModernOpenShiftRoute_UsesSessionTenantAndReturnsDecimalSafeShift(t *testing.T) {
	orgID, stationID, actorID := uuid.New(), uuid.New(), uuid.New()
	service := &modernShiftStub{result: appshift.Shift{ShiftID: uuid.New(), OrgID: orgID, StationID: stationID, StationSeq: 4, SupervisorID: actorID, OpenedAt: time.Date(2026, 9, 14, 3, 0, 0, 0, time.UTC), Status: appshift.StatusOpen}}
	...
	router := NewRouterWithDependencySet("test", nil, RouterDependencies{Readiness: readyStub{current: true}, Verifier: verifierStub{}, Sessions: sessions, ModernShift: service, LatestMigration: 11})
...
type modernShiftStub struct {
	request appshift.OpenRequest
	result  appshift.Shift
}

func (s *modernShiftStub) OpenShift(_ context.Context, request appshift.OpenRequest) (appshift.Shift, error) {
	s.request = request
	return s.result, nil
}

var _ ModernShiftService = (*modernShiftStub)(nil)
