...
func TestOpenAPIContract_ContainsVersionedRoutes(t *testing.T) {
	...
	for _, path := range []string{"/api/v1/login", "/api/v1/session", "/api/v1/shifts", "/api/v1/drafts/claim", "/api/v1/submissions", "/api/v1/reports/{id}", "/api/v1/reports/{id}/acknowledgement", "/api/v1/policies/revisions", "/health", "/ready"} {
