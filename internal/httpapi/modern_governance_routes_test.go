...
func TestModernAcknowledgeRoute_UsesVerifiedRoleAndScope(t *testing.T) {
	orgID, stationID, actorID, shiftID, reportID := uuid.New(), uuid.New(), uuid.New(), uuid.New(), uuid.New()
	service := &modernGovernanceStub{}
	sessions := &modernSessionStub{view: SessionView{OrgID: orgID, UserID: actorID, Roles: []string{"Station Admin"}, StationIDs: []uuid.UUID{stationID}}}
	router := NewRouterWithDependencySet("test", nil, RouterDependencies{Readiness: readyStub{current: true}, Verifier: verifierStub{}, Sessions: sessions, ModernGovernance: service, LatestMigration: 11})
...
type modernGovernanceStub struct {
	request appgovernance.AcknowledgeRequest
}

func (s *modernGovernanceStub) Acknowledge(_ context.Context, request appgovernance.AcknowledgeRequest) (appgovernance.Acknowledgement, error) {
	s.request = request
...
var _ ModernGovernanceService = (*modernGovernanceStub)(nil)
