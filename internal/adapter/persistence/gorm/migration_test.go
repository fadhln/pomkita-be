...
func TestCleanMigrationSet_DefinesAcknowledgementTables(t *testing.T) {
	root := repositoryRoot(t)
	path := filepath.Join(root, "migrations", "clean", "000007_governance_ack.up.sql")
	contents, err := os.ReadFile(path)
	if err != nil {
...
func TestCleanMigrationSet_AppliesAndReversesInAnIsolatedSchema(t *testing.T) {
	...
	if remaining != 42 {
		t.Fatalf("clean table count after reapply: got %d, want 42", remaining)
	}
	if err := runner.Down(ctx); err != nil {
