	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err != nil {
			...
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("org_id = ?", request.OrgID).First(&lock).Error; err != nil {
				return fmt.Errorf("lock new audit chain: %w", err)
			}
		}
		var last AuditLogModel
		if err := tx.Where("org_id = ?", request.OrgID).Order("org_sequence desc").First(&last).Error; err != nil && err != gorm.ErrRecordNotFound {
			return fmt.Errorf("load audit tail: %w", err)
		}
		...
		if err := tx.Create(&outbox).Error; err != nil {
			return fmt.Errorf("append audit outbox: %w", err)
		}
		...
		result = appaudit.Event{EventID: request.EventID, OrgSequence: sequence, RowHash: append([]byte(nil), rowHash...)}
		return nil
	})
	if err != nil {
