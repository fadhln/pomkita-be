...
func (r *ShiftRepository) OpenShift(ctx context.Context, request appshift.OpenRequest) (appshift.Shift, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if _, err := auditrepository.AppendInTransaction(tx, appaudit.AppendRequest{OrgID: request.OrgID, EventID: shiftID, EventType: "shift.opened", Payload: auditPayload, Outcome: "success"}, openedAt); err != nil {
...
// DecimalAddTenth returns a decimal meter value increased by one tenth.
func DecimalAddTenth(value string) string {
	return decimalAddTenth(value)
}
