...
func (r *GovernanceRepository) Acknowledge(ctx context.Context, request appgovernance.AcknowledgeRequest, now time.Time) (appgovernance.Acknowledgement, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if _, err := auditrepository.AppendInTransaction(tx, appaudit.AppendRequest{OrgID: request.OrgID, EventID: ack.AckID, EventType: "report.acknowledged", Payload: auditPayload, Outcome: request.Decision}, now); err != nil {
...
func (r *GovernanceRepository) RequestAmendment(ctx context.Context, request appgovernance.AmendmentRequest, now time.Time) (appgovernance.Amendment, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if _, err := auditrepository.AppendInTransaction(tx, appaudit.AppendRequest{OrgID: request.OrgID, EventID: amendment.AmendmentID, EventType: "amendment.requested", Payload: auditPayload, Outcome: "success"}, now); err != nil {
...
func (r *GovernanceRepository) RejectAmendment(ctx context.Context, request appgovernance.RejectAmendmentRequest, now time.Time) error {
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if _, err := auditrepository.AppendInTransaction(tx, appaudit.AppendRequest{OrgID: request.OrgID, EventID: uuid.New(), EventType: "amendment.rejected", Payload: auditPayload, Outcome: "rejected"}, now); err != nil {
...
func (r *GovernanceRepository) ApproveAmendment(ctx context.Context, request appgovernance.ApproveAmendmentRequest, now time.Time) (appgovernance.Amendment, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if _, err := auditrepository.AppendInTransaction(tx, appaudit.AppendRequest{OrgID: amendment.OrgID, EventID: newReportID, EventType: "amendment.approved", Payload: auditPayload, Outcome: "approved"}, now); err != nil {
