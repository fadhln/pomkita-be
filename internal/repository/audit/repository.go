...
func (r *AuditRepository) Append(ctx context.Context, request appaudit.AppendRequest, now time.Time) (appaudit.Event, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var err error
		result, err = AppendInTransaction(tx, request, now)
		return err
	})
	if err != nil {
...
// AppendInTransaction appends an audit event in an existing database transaction.
func AppendInTransaction(tx *gorm.DB, request appaudit.AppendRequest, now time.Time) (appaudit.Event, error) {
...
// AuditRowHash calculates the immutable digest for an audit row.
func AuditRowHash(orgID uuid.UUID, sequence int64, eventID uuid.UUID, eventType string, payload []byte, createdAt time.Time, prevHash []byte) []byte {
	return auditRowHash(orgID, sequence, eventID, eventType, payload, createdAt, prevHash)
}
