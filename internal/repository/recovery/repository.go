...
func NewRecoveryRepository(store *store.Store) *RecoveryRepository {
	if store == nil {
...
func (r *RecoveryRepository) RecoverStale(ctx context.Context, now time.Time, maxAge time.Duration) (int, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var drafts []store.ShiftDraftModel
		...
		for _, draft := range drafts {
			if err := tx.Model(&store.ShiftDraftModel{}).Where("draft_id = ? and status = ?", draft.DraftID, "submitting").Updates(map[string]any{"status": "recovering", "recovery_count": gorm.Expr("recovery_count + 1"), "updated_at": now}).Error; err != nil {
			...
			if !hasReport {
				if err := tx.Model(&store.ShiftDraftModel{}).Where("draft_id = ?", draft.DraftID).Updates(map[string]any{"status": "editing", "updated_at": now}).Error; err != nil {
					return fmt.Errorf("reopen draft: %w", err)
				}
				if err := tx.Model(&store.ShiftModel{}).Where("shift_id = ? and status = ?", shift.ShiftID, "submitting").Updates(map[string]any{"status": "open", "closed_at": nil}).Error; err != nil {
					return fmt.Errorf("reopen shift: %w", err)
				}
			} else {
				if err := tx.Model(&store.ShiftDraftModel{}).Where("draft_id = ?", draft.DraftID).Updates(map[string]any{"status": "submitted", "updated_at": now}).Error; err != nil {
					return fmt.Errorf("complete recovered draft: %w", err)
				}
				if err := tx.Model(&store.ShiftModel{}).Where("shift_id = ? and status = ?", shift.ShiftID, "submitting").Updates(map[string]any{"status": "awaiting_confirmation"}).Error; err != nil {
