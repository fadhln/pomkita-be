...
func TestPersistenceModels_UseExplicitTableNames(t *testing.T) {
	cases := []struct {
		...
		{name: "alert rule", model: AlertRuleModel{}, want: "alert_rules"},
		{name: "alert event", model: AlertEventModel{}, want: "alert_events"},
		{name: "audit chain lock", model: AuditChainLockModel{}, want: "audit_chain_locks"},
		{name: "audit log", model: AuditLogModel{}, want: "audit_log"},
		{name: "audit outbox", model: AuditOutboxModel{}, want: "audit_outbox"},
		{name: "audit denied", model: AuditDeniedModel{}, want: "audit_denied"},
		{name: "relay state", model: OutboxRelayStateModel{}, want: "outbox_relay_state"},
