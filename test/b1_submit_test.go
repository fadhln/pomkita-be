...
func TestB1Submit_ReportAndPolicyObjectsExist(t *testing.T) {
	...
	if _, err := conn.Exec(ctx, readMigration(t, repositoryRoot(t), "000008_b1_submit.down.sql")); err != nil {
...
func TestB1Submit_RolloverAndReplay(t *testing.T) {
	...
	for _, s := range []struct {
		...
		{`insert into users(user_id,org_id,email,display_name,password_hash) values($1,$2,'submit@example.com','Supervisor',app.crypt('pw', app.gen_salt('bf')))`, []any{user, org}},
