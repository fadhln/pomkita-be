...
drop function if exists public.fn_set_request_context(text);
alter table public.sessions drop column if exists last_active_at;
...
begin
  ...
  begin
    v_header_text := convert_from(decode(replace(replace(v_parts[1], '-', '+'), '_', '/') || repeat('=', (4 - length(v_parts[1]) % 4) % 4), 'base64'), 'UTF8');
    v_claims_text := convert_from(decode(replace(replace(v_parts[2], '-', '+'), '_', '/') || repeat('=', (4 - length(v_parts[2]) % 4) % 4), 'base64'), 'UTF8');
    v_header := v_header_text::jsonb;
    v_claims := v_claims_text::jsonb;
    v_signature := decode(replace(replace(v_parts[3], '-', '+'), '_', '/') || repeat('=', (4 - length(v_parts[3]) % 4) % 4), 'base64');
  ...
  if not ((jsonb_typeof(v_claims->'aud') = 'string' and v_claims->>'aud' = 'spbu-recon') or (jsonb_typeof(v_claims->'aud') = 'array' and v_claims->'aud' @> '["spbu-recon"]'::jsonb)) then
  ...
  select k.secret_ref, k.status, k.max_token_expiry into v_key_secret_ref, v_key_status, v_key_max_token_expiry from public.jwt_keys k where k.kid = v_kid;
  if not found then
    raise exception using errcode = '28000', message = 'jwt_unknown_kid';
  end if;
  if v_key_status = 'retired' or (v_key_status = 'previous' and v_key_max_token_expiry <= clock_timestamp()) then
  ...
  select s.issued_at, s.expires_at, s.revoked_at into v_session_issued_at, v_session_expires_at, v_revoked_at from public.sessions s where s.jti = v_jti and s.kid = v_kid;
  if not found then
  ...
  if v_session_expires_at < clock_timestamp() - interval '60 seconds' or v_session_issued_at > clock_timestamp() + interval '1 second' then
    raise exception using errcode = '28000', message = 'jwt_session_expired';
  end if;
  select u.org_id, u.enabled into v_org_id, v_enabled from public.users u where u.user_id = v_subject;
  if not found then
  ...
  select string_agg(usr.role, ',' order by usr.station_id, usr.role), min(usr.role) into v_roles, v_role from public.user_station_roles usr where usr.org_id = v_org_id and usr.user_id = v_subject;
