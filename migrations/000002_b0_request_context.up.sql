...
begin
  ...
  if v_key_status = 'retired'
     or (v_key_status = 'previous' and v_key_max_token_expiry <= clock_timestamp()) then
  ...
  select s.issued_at, s.expires_at, s.revoked_at
    into v_session_issued_at, v_session_expires_at, v_revoked_at
  ...
  if v_session_expires_at < clock_timestamp() - interval '60 seconds'
     or v_session_issued_at > clock_timestamp() + interval '1 second' then
