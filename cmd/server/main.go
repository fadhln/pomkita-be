...
func main() {
	...
	router := httpapi.NewRouterWithAllDependencies(cfg.Environment, cfg.CorsAllowedOrigins, database, jwtService, sessionManager, appdb.NewShiftManager(database), appdb.NewGovernanceManager(database), appdb.NewReportingManager(database))
